...
void ProblemInstance::update(std::vector<sf::CircleShape>& nCircles, std::vector<std::pair<double, sf::Vertex>>& alledges)
{   
...
void ProblemInstance::render(sf::RenderWindow& window, const std::vector<sf::CircleShape>& nodes/*, const std::vector<std::pair<double, sf::Vertex>>& edges*/)
{
    ...
    std::vector<std::pair<double, sf::Vertex>> transformedEdges;
    for (size_t i = 0; i < alledges.size(); ++i) {
        auto pos = alledges[i].second.position;
        sf::Vector2f newPos(pos.x * scaleX + offset.x, pos.y * scaleY + offset.y);
        if(!(newPos.x == 0 && newPos.y == 0))
        {
            transformedEdges.push_back({alledges[i].first, sf::Vertex(newPos, alledges[i].second.color)});
        }
    ...
    {
    //    window.draw(&transformedEdges[0].second, transformedEdges.size(), sf::Lines);
     //   std::cout << transformedEdges.size() << std::endl;
      //  sf::Text label;
      //  label.setFont(font);
      ...
      */
        for (size_t i = 0; i + 1 < transformedEdges.size(); i += 2) {
            // Draw the edge (2 vertices at a time)
            Logger::info("V1:(" + std::to_string((double)alledges[i].second.position.x) + "," + std::to_string((double)alledges[i].second.position.y) + ")", "drawlog.txt");
            Logger::info("V2:(" + std::to_string((double)alledges[i+1].second.position.x) + "," + std::to_string((double)alledges[i+1].second.position.y) + ")", "drawlog.txt");

            Logger::info("DrawingV1:(" + std::to_string((double)transformedEdges[i].second.position.x) + "," + std::to_string((double)transformedEdges[i].second.position.y) + ")", "drawlog.txt");
            Logger::info("DrawingV2:(" + std::to_string((double)transformedEdges[i+1].second.position.x) + "," + std::to_string((double)transformedEdges[i+1].second.position.y) + ")", "drawlog.txt");

            window.draw(&transformedEdges[i].second, 2, sf::Lines);
            
            // Compute the midpoint between the two vertices
       //     sf::Vector2f p1 = transformedEdges[i].second.position;
      //      sf::Vector2f p2 = transformedEdges[i + 1].second.position;
